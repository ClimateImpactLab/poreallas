...
def _(isku, np, numba, xr):
    ...
    def _hotdeath_noadapt_beta_from_gamma(ds: xr.Dataset) -> xr.Dataset:
        """
        For hot-related deaths only, calculates mortality impact polynomial model's beta coefficients from gamma coefficients for the no-adaptation scenario.
        ...
        beta_hot = beta.where(beta["tas_bin"] > mmt, other=0)

        beta.attrs = {
            "units": "deaths per 100,000 people",
            "long_name": "Hot-death temperature mortality rate",
        }

        # Returns new dataset with beta added as new variable. Not modifying
        # original ds. Also ensure original data is passed through to projection.
        return ds.assign(beta=beta_hot)
    ...
    mortality_hotdeath_effect_model = isku.build_projection_template(
        pre=_hotdeath_noadapt_beta_from_gamma,
        project=_mortality_effects_model,
        post=_no_processing,
    )
    return (mortality_hotdeath_effect_model,)
...
@app.cell
def _(forecast_input_ds, isku, mortality_hotdeath_effect_model):
    projected_forecast = isku.project(forecast_input_ds, model=mortality_hotdeath_effect_model)
...
@app.cell
def _(hist_input_ds, isku, mortality_hotdeath_effect_model):
    projected_hist = isku.project(hist_input_ds, model=mortality_hotdeath_effect_model)
