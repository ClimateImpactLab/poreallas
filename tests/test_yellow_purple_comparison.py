...
These tests were first adapted from the prototype at https://github.com/brews/ideal-succotash/blob/c30a9d8226aaef806a16c5fdae3c186761811829/tests/mortality/test_yellow_purple_comparison.py.
"""
...
def input_dataset():
    ...
    out = xr.Dataset(
        {
            "climtas": (["year"], np.array([10.69912])),
            "loggdppc": (["year"], np.array([10.83039802])),
            "gamma": (
                ["degree", "covarname"],
                np.array(
                    [
...
def test_noadapt_beta(input_dataset):
    ...
    expected = xr.DataArray(
        ...
        coords=[np.array([2015]), np.linspace(-40, 40, 9, dtype="float32")],
        dims=["year", "tas_bin"],
        name="beta",
    )
